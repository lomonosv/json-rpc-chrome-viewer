...
- **Waterfall column** showing each request's start offset and duration on a shared timeline
- **Sortable columns** — click any header to sort ascending/descending
- **Configurable columns** — show or hide Waterfall, Status, Size and Time
- Filter by method name
- Keyboard navigation with <kbd>↑</kbd> / <kbd>↓</kbd>
...
- Configurable default expand/collapse depth, separately for HTTP and WebSocket messages
...
Select a row to see its request and response. The gear icon opens Settings.
...
| Appearance | Theme · Show url for each request · Show CORS badge · Show Websocket badge · Columns (Waterfall, Status, Size, Time) · JSON Tree Viewer Theme · JSON Tree Open State · JSON Tree Open State (Websocket Messages) |
| Filters | Include JSON-RPC Logs · Include Websocket Logs |

Settings persist in `chrome.storage.local`. The Method column is always shown, since it identifies the row.

## Permissions

| Permission | Why |
| --- | --- |
| `storage` | Persist settings and pane sizes |
