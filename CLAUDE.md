...
All five normalise into `IRequest` (`src/logic/HTTPArchive/IRequest.ts`), discriminated by `isWebSocket`. Detection is a regex for `"jsonrpc": "2.0"` against the raw body, not a JSON parse, so it tolerates SockJS's double-encoded string frames — `parseJsonRpcMessage()` unwraps those. `isJsonRpcMessage()` also checks the *type* first, and must keep doing so: `websockets.ts` relays every frame raw, so a binary frame (ArrayBuffer, Blob, typed array) reaches the panel as a non-string. Calling `.match` on one threw inside the `onMessage` listener — "Error in event handler: TypeError: e.match is not a function" — on any page whose sockets carry binary frames. Binary frames are skipped, not decoded; JSON-RPC sent as a binary frame is therefore not captured. **JSON-RPC batches are exploded into one `IRequest` per batch item**, with responses correlated back by `id`.
...
Breakpoints are defined **twice on purpose** — `src/logic/common/breakpoints.ts` for the parts that adapt in JS and `$narrowWidth` in `variables.scss` for the parts that adapt in CSS. Keep the pair in sync. The SCSS side is only ever consumed through the `narrowLayout` mixin, so a breakpoint change lands in one place per realm. `media-query-no-invalid` is disabled in `.stylelintrc.js` because it cannot parse a Sass variable inside a media feature; that mixin is the only media query in the project. There is no compact breakpoint any more: the toolbar, its only user, now fits itself by measurement (below).
...
- **The toolbar keeps as many controls as fit - measured, not breakpointed.** Only the four controls that are also in Settings → Preferences may leave it - Preserve log and the three include toggles - and that is the condition for hiding one, not a nicety; it makes **the gear the one control that must never leave the screen.** `useToolbarFit` (`Toolbar/useToolbarFit.ts`) sums the real outer widths of the fixed controls, counting the filter at `comfortableFilterWidth`, and adds the optional ones in toolbar order while they fit, so they drop **right to left**: Server Logs, Websocket Logs, JSON-RPC Logs, Preserve log. The filter gives way first, from 160px down to that comfortable width, before any toggle hides.

  It replaced two pixel breakpoints, `compactHidden` (1120px, all three include toggles at once) and `narrowHidden` (620px, Preserve log). Both were tuned to one font - the compact one had already been 900px, which let "Include Server Logs" paint over the gear on a real 964px side-docked panel - and all-or-nothing steps hid three toggles where one would have done. Do not reintroduce breakpoints for this. These are load-bearing:
  - **Overflowing controls leave the flow with `visibility: hidden` + `position: absolute`, never `display: none`.** They stay measurable, so the count depends only on widths and cannot oscillate as controls appear and disappear; `visibility` also takes them out of the tab order and the accessibility tree. The rule is `.isOverflowing.isOverflowing` because `Input`'s own `.inputWrapper { position: relative }` is in another stylesheet at the same specificity.
  - **The observer watches every control, not only the section.** The section's width is fixed by the panel - it flex-grows into the space - but the scope select resizes with its selected option.
  - **The filter's shrink rule sits on `.filterWrapper`**, the `<label>` `Input` wraps around the field. The label is the flex item; with the rule on the input it kept ~160px and pushed the gear off a 360px panel. Its `min-width` (130px) mirrors `comfortableFilterWidth` - keep them in step - and drops to 60px on a narrow panel only as a last resort, once every toggle has already gone.
  - **`.layoutWrapper` carries `min-width: 0`.** It is a flex item of `#application`, and the default `min-width: auto` let the whole panel grow to its widest unshrinkable child (406px inside 360) rather than making that child give way.

  Headless Chrome will not size a window below 500px, so a narrow check has to frame the panel in an iframe of the target width - its media queries read the frame's viewport. A 420px screenshot of a 500px window is a crop, not a narrow layout.
